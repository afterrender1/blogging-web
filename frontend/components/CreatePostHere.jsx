            ...
            <div className="w-full max-w-3xl bg-white rounded-xl p-6 sm:p-10 lg:p-12 border border-gray-200">
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {/* 1. Post Title */}
                        <div>
                            <label htmlFor="title" className="block text-sm font-semibold mb-2 text-gray-700 ">Post Title</label>
                            <input
                                id="title"
                                type="text"
                                value={title}
                        ...
                        <div>
                            <label htmlFor="intro" className="block text-sm font-semibold mb-2 text-gray-700 ">Short Introduction (Teaser)</label>
                            <textarea
                                id="intro"
                                rows="2"
                                value={intro}
                    ...
                    {/* 3. Full Content */}
                    <div>
                        <label htmlFor="content" className="block text-sm font-semibold mb-2 text-gray-700 ">Full Blog Content</label>
                        <textarea
                            id="content"
                            rows="12"
                            value={content}
                    ...
                    <div>
                        <label htmlFor="image-upload" className="block text-sm font-semibold mb-2 text-gray-700 ">Featured Image</label>
                            ...
                            <label 
                                ...
                                className={`cursor-pointer flex flex-col items-center justify-center w-full h-36 border-2 border-dashed rounded-lg transition p-4
                                            ${imageFile 
                                                ...
                                                : 'border-gray-400 hover:bg-gray-50'}`} // Subtle hover on flat design
                            >
                                <svg className="w-9 h-9 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                <p className={`mt-2 text-sm font-medium ${imageFile ? 'text-green-700' : 'text-gray-600'}`}>
                                    {imageFile ? `File Selected: ${imageFile.name}` : 'Drag & Drop or Click to upload JPG/PNG'}
                                </p>
                                <input 
                                    id="image-upload" 
                                    type="file" 
